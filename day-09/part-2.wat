(module
    ...
    (import "dbg" "inspect:swap" (func $dbg.swap (param i32 i32) (result i32 i32)))
    ...
    (func $solve_line
        ...
        ;; (drop (call $dbg.sequence (global.get $input_len)))
        (call $reverse_line (local.get $len))
        ;; (drop (call $dbg.sequence (global.get $input_len)))
    ...
    (func $reverse_line (param $len i32)
        (local $idx i32)
        (local $opposite_idx i32)
        (local $half i32)
        (local $tmp i32)
        ...
        (loop $continue
            ...
            ;; (drop (drop (call $dbg.swap (local.get $idx) (local.get $opposite_idx))))

            (local.set $tmp
                (call $load_location
                    (local.get $opposite_idx)))
            (call $store_location
                (local.get $opposite_idx)
                (call $load_location
                    (local.get $idx)))
